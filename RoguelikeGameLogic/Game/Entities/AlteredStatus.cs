...
namespace RoguelikeGameEngine.Game.Entities
{
    public class AlteredStatus : Entity, IHasActions
    {
        ...
        {
            if (!CanOverwrite && target.AlteredStatuses.Any(als => als.ClassId.Equals(ClassId))) return false;
            if (!CanStack && target.AlteredStatuses.Any(als => als.ClassId.Equals(ClassId)))
            {
